// Вам дана строка «abc» и предполагая, что каждая буква в строке имеет значение, равное ее позиции в алфавите, то наша строка будет иметь значение 1 + 2 + 3 = 6.
// Это означает, что: a = 1, b = 2, c = 3 .... z = 26.
// Вам будет предоставлен список строк, и ваша задача будет вернуть значения строк, умноженные на позицию этой строки в списке.
...
void main() {
  List words = ["dart", "abc", "good luck"];//Список слов
  List result = [];
  String alphabet = 'abcdefghijklmnopqrstuvwxyz'; // Алфавит
  words.asMap().forEach((index, word) => {
    result.add(Split(alphabet, word, index +1))
  });
  print(result);
}

int Split(String alphabet, String chars, int index) {
  int counter = 0;
  for(var i=0; i<chars.length;i++){
    counter += alphabet.indexOf(chars[i])+1;
  }
